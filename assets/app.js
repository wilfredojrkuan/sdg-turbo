...
selectCallback = function(variant, selector) {
  ...
  if($(this).val() == "ECONOMICAL - Deposit Now, Rest Later."){ // or this.value == 'volvo'
    ...
    $("#myModal").css("display","block");
     ...
     $("#eco-bal").css("display", "none");
      $("#eco-note").css("display", "none");
    }
});
  
  $('select.single-option-selector').change(function(){
  if($(this).val() == "WHITE GLOVE SERVICE - Expedited Delivery, Free Storage Case & More!"){ // or this.value == 'volvo'
     $(".wgs-torso").css("display","block");
    
  }
    else {
     $(".wgs-torso").css("display", "none");
    }
});
  ...
  if (variant && variant.available == true) {
    if(variant.price < variant.compare_at_price){
      ...
      $('.savings', $product).html("At SDG, You Save" + ' ' + parseInt(((variant.compare_at_price - variant.price) * 100) / variant.compare_at_price) + '% (' + '<span class="money">' + Shopify.formatMoney(variant.compare_at_price - variant.price, $productForm.data('money-format')) + '</span>)');
        ...
        //No variants
          ...
          if (product.available == true) {
            ...
            if (product.compare_at_price > product.price) {
              ...
              $('.js-savings').html("At SDG, You Save" + ' ' + parseInt(((product.variants[0].compare_at_price - product.variants[0].price) * 100) / product.variants[0].compare_at_price) + '% (' + '<span class="money">' + Shopify.formatMoney(product.variants[0].compare_at_price - product.variants[0].price, product.money_format) + '</span>)');
            }
            ...
            if (variant == product.variants[i].id.toString()){
              ...
              if (product.variants[i].available == true) {
                ...
                if (product.variants[i].compare_at_price > product.variants[i].price) {
                  ...
                  $('.js-savings').html("At SDG, You Save" + ' ' + parseInt(((product.variants[i].compare_at_price - product.variants[i].price) * 100) / product.variants[i].compare_at_price) + '% (' + '<span class="money">' + Shopify.formatMoney(product.variants[i].compare_at_price - product.variants[i].price, product.money_format) + '</span>)');
                }
...
function checkContainer () {
  if($('.btn-beatanyprice').is(':visible')){ //if the container is visible on the page
    $( ".btn-beatanyprice" ).first().css( "display", "none" );
  } else {
...
$('body').on('click', '.mw_product_option_label', function () {
   $('.mw-options-container .mw_options_list').addClass('swatch-option');
    var parent = $(this).closest('.mw-option');
  
    var swatch = parent.find('.swatch-option');

    $('.swatch-option').css('display', 'none');
 	 $('.mw_option_list').css('display', 'block');

    if (current_option === this && is_open) {
        is_open = false;
      	$(this).removeClass('active');
        return swatch.css('display', 'none');
    ...
    return swatch.css('display', 'block');
});

  $('#btnAddToCart').on('click', function () {
  	$('#da_form').find('#mwAddToCart').click();
  });
  $('#btnAddToCartNow').on('click', function () {
  	$('#da_form').find('#mwAddToCart').click();
  });

  $('#btnBuyNow').on('click', function () {
  	$('#da_form').find('[data-testid="Checkout-button"]').click();
  });
...
//     var li = $('label.mw_product_option_label').addClass('icon-down-arrow');
// setTimeout(function () {
//     li.removeClass('go');
// }, 4500);
// });
...
(function($) {
  ...
  // Open Lightbox
  $('.lightbox-enabled').on('click', 'img', function(e) {
    e.preventDefault();
